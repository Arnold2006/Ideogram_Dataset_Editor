...
const { IDEOGRAM_SCHEMA, KEY_ORDER } = require('./ideogram-schema');
    ...
    if (typeof comp === "object" && comp !== null && !Array.isArray(comp)) {
      ...
      if (Array.isArray(comp.elements)) {
        comp.elements.forEach((element, i) => {
          ...
          const order = element.type === "text" ? KEY_ORDER.elementText : KEY_ORDER.elementObj;
          ...
          if (Array.isArray(element.bbox) && element.bbox.length === 4) {
            ...
            if (yMin > yMax || xMin > xMax) {
              errors.push(`elements[${i}].bbox: expected y_min <= y_max and x_min <= x_max`);
            }
          }
        });
      }
